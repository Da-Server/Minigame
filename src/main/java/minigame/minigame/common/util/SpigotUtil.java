...
public class SpigotUtil {
    ...
    public static boolean isSimilar(ItemStack i, ItemStack j) {
        ItemMeta iMeta = i.getItemMeta();
        ItemMeta jMeta = j.getItemMeta();

        if(!iMeta.getDisplayName().equals(jMeta.getDisplayName())) return false;
        if(i.getType() != j.getType()) return false;
        if (i.getMaxStackSize() != j.getMaxStackSize()) return false;

        return true;
    }


    //<- https://www.spigotmc.org/threads/send-titles-to-players-using-spigot-1-8-1-11-2.48819/ - for sending titles.
    /**
